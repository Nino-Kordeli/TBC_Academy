...
import com.example.tbcacademy.domain.usecase.auth.ValidateLoginUseCase
...
import com.example.tbcacademy.presentation.login.contract.LoginState
    ...
    private fun login() = viewModelScope.launch {
        ...
        loginUseCase(email, password).collect { result ->
            when (result) {
                ...
                is Resource.Error -> {
                    updateState { it.copy(loading = false, error = result.errorMessage) }
                    emitSideEffect(LoginSideEffect.ShowSnackBar(result.errorMessage))
                }
