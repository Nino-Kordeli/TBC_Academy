...
class AuthRepositoryImpl @Inject constructor(
    ...
    override fun login(request: AuthRequest): Flow<Result<LoginResponse>> = flow {
        emit(Result.Loading)
        val result = safeApiCall { api.login(AuthRequestDto(request.email, request.password)) }
    ...
    override suspend fun register(request: AuthRequest): Result<RegisterResponse> {
        val result = safeApiCall { api.register(AuthRequestDto(request.email, request.password)) }
